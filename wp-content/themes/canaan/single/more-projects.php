...
<div class="hidden lg:flex w-full  flex-col gap-y-5 px-4">
    ...
    <div class="flex  justify-center gap-x-3 gap-y-5 mt-5 mb-5 max-w-[1396px]">
        ...
        foreach ($projects as $key => $p) {
            ...
            echo '<a class="w-full  h-[340px] group transition" href="' . get_the_permalink($p) . '">';
            ...
            echo get_img_html($image, true, 'full', 'w-full h-full object-contain group-hover:scale-105 transition ease-in-out duration-500');
            echo '</div>';
            echo '</a>';
        }
        ?>
    ...
    <!-- </div> -->
    <a class="border max-w-xs border-blue-400 mt-8 py-3 px-3 text-blue-400 text-2xl font-semibold montserrat rounded-md self-center hover:bg-slate-100 focus:outline-none focus:ring focus:ring-blue-300 " href="<?php echo get_post_type_archive_link($post_type)  ?> ">
        <?= pll__('All'); ?>
        <?= $post_type; ?>s
    </a>
</div>